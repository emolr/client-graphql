...
export const Pokemon: QueryResolvers['pokemon'] = async (root, { id }, { http }) => {
    try {
...
export const Pokemons: QueryResolvers['pokemons'] = async (root, { offset, first }, { http }) => {
    try {
        const offsetParameter = offset ? offset : 0;
        const limitParameter = first ? first : 20;
        const { data } = await http.get(`pokemon?offset=${offsetParameter}&limit=${limitParameter}`)
